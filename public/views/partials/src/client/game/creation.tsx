...
export default class Creation extends React.Component<Props, State> {
	...
	handleSubmission = () => {
		if (this.state.freePts > 0) {
			tpl.modal("error", "Ooops!", "You still have free points to distribute!");
			return;
		}
		...
		const obj = {
			...
			background: this.props.backstory,
		};

		this.props.socket.emit("player-create", obj);
		return;
	}

	componentDidMount() {
		this.props.socket.on("player-create-response", (data) => {
			if (data.status == "ok") {
				this.props.handleSubmit();
			} else {
				tpl.modal("error", "Ooops!", "Something went terribly wrong!");
			}
		});
	}

	render() {
		const backstory = this.props.backstory;

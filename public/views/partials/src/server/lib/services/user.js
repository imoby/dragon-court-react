...
class UserService {
  ...
  async create(data) {
  	...
  	if(error != null){
  		...
  		const token = jwt.sign({data: data.id}, this.core.config.secret, { expiresIn: 60*60 });
  		const q = 'INSERT INTO users SET email = ?, name = ?, pass = ?, token = ?';
  		const results = this.core.db.query(q, [data.email, data.username, hash, token]);
  ...
  async login(data) {
  	...
  	if(results.length > 0){
  		...
  		if(!verified){
  			...
  			const token = jwt.sign({data: results[0].id}, this.core.config.secret, {expiresIn: 60*60});
